...
int main(int argc, char* argv[]) {
    ...
    ssize_t sent_bytes = rudp_send_data(sockfd, message, strlen(message) + 1, server_addr);
    if (sent_bytes > 0) {
    ...
    ssize_t received_bytes = rudp_receive_data(sockfd, buffer, DATA_SIZE, server_addr);
    if (received_bytes > 0) {
